...
| UI-028 | FOOTPRINT / LEVEL 2 is off by default. Enabling it requests only the visible chart window from the compact order-flow endpoint. Zoomed-in view renders white buyer/red seller fill-only bubbles, with radius bands tied to 50–99 / 100–149 / 150+ quantities, passive activity encoded by fill opacity, imbalance markers, and the strongest bounded depth levels; overview view aggregates cells per screen bucket, keeps one dominant cell per screen column, applies a 50-contract minimum when available, and defers detailed bands. Panning is debounced and disabling the layer clears its canvas | `test_orderflow.py` + `test_ui_glass_repair_contracts.py` |
