...
| PI-007 | 重複的外部事件不得產生重複的策略動作；跨歷史／audit／repost 來源以「1m 來源時間 + 商品 + kind」去重，不得只依賴 Discord message ID | `test_pi_single_source.py` + `test_pi_live_audit.py` |
...
| PI-010 | The single active **new-channel** Discord listener's `received`/`recorded` rows may overlay Live and Backtest charts; the overlay must be deduped against history by 1m source time + symbol + kind, and rendering never mutates immutable history | `test_ui_glass_repair_contracts.py` + `test_pi_live_audit.py` |
| PI-011 | An explicit PI Backtest may merge in-range **new-channel** audit rows into that run only (deduped by 1m source time + symbol + kind and pre-session filtered); it never appends to `pi_history`, changes Live callbacks, or creates an order | `test_pi_live_audit.py` + `test_pi_pre_session_filter.py` |
...
| CONFIG-007 | 儲存、計算與 API instant 一律以 UTC 為基準；市場 session 使用 DST-aware New York、Topstep 日界使用 Chicago 17:00、PI source 使用 Los Angeles。前端圖表只在顯示時依瀏覽器／作業系統本地時區位移。Production contract default 必須動態解析 front month，不得寫死 M26/U26 | `test_timebase_contract.py` + `test_market_clock.py` + `tests/ui/chart-catchup.spec.js` |
