...
| ID | 不變量 | Test |
|---|---|---|
| LIVE-001 | 用歷史 K 棒熱身後,**必須** `reset_to_safe_state()`。殘留的 zone 資料會造成瞬間錯價成交 | `UNPROTECTED` ⚠️ |
| LIVE-002 | 安全檢查必須 **BLOCK**(return),不得只 WARN 後繼續 | `UNPROTECTED` ⚠️ |
...
| ID | 不變量 | Test |
|---|---|---|
| CONFIG-001 | 新策略**必須**加進 `FACTOR_PIPELINE_STRATEGIES` / `ZONELESS_STRATEGIES`(models.py)。漏加 = 回測與實盤靜默不一致(實測 PnL 差 33%)且慢 20 倍 | `UNPROTECTED` ⚠️ |
...
**33 條裡有 21 條是 `UNPROTECTED`。** 標 ⚠️ 的六條是我認為最該優先補的,
因為它們的失敗模式是**靜默的**且直接影響金錢或資料正確性:

```
LIVE-001  熱身後未重置 → 瞬間錯價成交
LIVE-002  檢查只 WARN 不 BLOCK → 護欄形同虛設
LIVE-004  休息時強制平倉 → 砍掉本來會獲利的部位
DATA-002  接縫未保留錨點 → 回測資料出現假跳空
DATA-003  用跳幅猜換月 → 把真實行情抹平(已發生過一次)
CONFIG-001 漏加白名單 → 回測與實盤靜默不一致
```
