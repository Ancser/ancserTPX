"""Rolling sigma fade study.

Corrected interpretation:
  - 15m/30m is the rolling calculation window for the current distribution.
  - Every new candle recalculates center/sigma from the previous window.
  - Signals are generated from the updated sigma bands, not from a fixed
    first-15-minute opening distribution.

Research-only. It does not modify live or the production backtest engine.

Run:
  PYTHONIOENCODING=utf-8 python -m scripts.rolling_sigma_fade_study
"""
...
OUT_TXT = OUT_DIR / "rolling_sigma_fade_study.txt"
OUT_JSON = OUT_DIR / "rolling_sigma_fade_study.json"
OUT_TRADES = OUT_DIR / "rolling_sigma_fade_best_trades.csv"
...
class Variant:
    ...
    def name(self) -> str:
        return (
            f"{self.session_set}|roll{self.window_minutes}|{self.sigma_method}|"
...
class SessionData:
    ...
    start: datetime
    candles: list[Candle]
    dists: dict[tuple[int, str], list[Optional[tuple[float, float]]]] = field(default_factory=dict)
...
def _dist_for_window(candles: list[Candle], window: int, method: str) -> list[Optional[tuple[float, float]]]:
    ...
    if method == "std":
        ...
        for p, w in zip(prices, weights):
            sw.append(sw[-1] + w)
            swp.append(swp[-1] + p * w)
            swp2.append(swp2[-1] + p * p * w)
...
def _build_sessions(candles: list[Candle]) -> list[SessionData]:
    ...
    for (code, start), bars in groups.items():
        if len(bars) < max(WINDOWS) + 1:
            continue
        s = SessionData(code=code, start=start, candles=bars)
        for window in WINDOWS:
            for method in SIGMA_METHODS:
                s.dists[(window, method)] = _dist_for_window(bars, window, method)
        sessions.append(s)
...
def _check_exit(pos: Position, candle: Candle) -> bool:
    ...
    if t.direction == Direction.BUY:
        ...
        if hit_sl and hit_tp:
            ...
            _close_trade(t, t.sl_price if first == "sl" else t.tp_price, candle.timestamp, ExitReason.SL if first == "sl" else ExitReason.TP)
            return True
        if hit_sl:
            _close_trade(t, t.sl_price, candle.timestamp, ExitReason.SL)
            return True
...
def _fade_candidate(
    ...
    short_pick = None
    if armed_short and not disable_short:
        for level in levels:
            entry = _round_tick(center + level * sigma)
            if candle.high >= entry and (variant.entry_mode == "blind" or candle.close <= entry):
    ...
    long_pick = None
    if armed_long and not disable_long:
        for level in levels:
            entry = _round_tick(center - level * sigma)
            if candle.low <= entry and (variant.entry_mode == "blind" or candle.close >= entry):
                target_l = _target_level(level, variant.target_mode)
                tp = _round_tick(center - target_l * sigma)
                sl = _round_tick(entry - variant.stop_span * sigma)
                if sl < entry and tp > entry:
...
def _trend_candidate(
    ...
    down_entry = _round_tick(center - ACCEPT_SIGMA * sigma)
    buy_pick = None
    sell_pick = None
    ...
    if accepted_down and "down" not in trend_used and candle.low <= down_entry <= candle.high:
        sl = _round_tick(down_entry + variant.stop_span * sigma)
        tp = _round_tick(down_entry - sigma)
        if tp < down_entry < sl:
            sell_pick = (Direction.SELL, down_entry, sl, tp, ACCEPT_SIGMA)
    if buy_pick and sell_pick:
        return None
    return buy_pick or sell_pick


def _simulate_session(session: SessionData, variant: Variant, trade_counter: int) -> tuple[list[Trade], int]:
    dists = session.dists[(variant.window_minutes, variant.sigma_method)]
    ...
    for i, candle in enumerate(session.candles):
        ...
        if down_count >= ACCEPT_BARS:
            accepted_down = True
        if candle.close <= center + variant.start_sigma * sigma:
            accepted_up = False
        if candle.close >= center - variant.start_sigma * sigma:
            accepted_down = False
        ...
        candidate = _trend_candidate(candle, center, sigma, variant, accepted_up, accepted_down, trend_used)
        kind = "trend"
        ...
        trade = _new_trade(f"RSF-{trade_counter}", direction, entry, sl, tp, candle.timestamp, meta)
...
def _split_segments(trades: list[Trade]) -> tuple[list[float], bool]:
    ...
    buckets = [set(dates[: cuts[0]]), set(dates[cuts[0] : cuts[1]]), set(dates[cuts[1] :])]
    pnls = [
        sum((t.pnl or 0.0) for t in trades if _topstep_trade_date(t.entry_time) in bucket)
...
def _worst_day(trades: list[Trade]) -> float:
    day: dict[str, float] = {}
    for t in trades:
        k = _topstep_trade_date(t.entry_time)
        day[k] = day.get(k, 0.0) + (t.pnl or 0.0)
    return min(day.values()) if day else 0.0


def _simulate_variant(variant: Variant, sessions: list[SessionData]) -> tuple[dict, list[Trade]]:
    ...
    accepted = bool(
        goal
        and int(metrics.total_trades) >= 80
        and pf >= 1.25
...
def main() -> None:
    ...
    lines = [
        ...
        "15m/30m means rolling lookback window; sigma is recalculated every candle.",
        "goal: PNL > 6000 and maxDD < 1000. accepted: goal + trades>=80 + PF>=1.25 + WF all positive.",
        "",
        f"goal_count: {len(goal)}",
        f"accepted_count: {len(accepted)}",
        f"positive_count: {len(positive)}",
        "",
        f"{'OK':<2} {'variant':<67} {'n':>5} {'win%':>7} {'pnl':>9} {'maxDD':>7} {'PF':>5} {'expect':>7} {'loss':>9} {'wDay':>8} WF {'seg1/seg2/seg3':>26}",
        "-" * 168,
    ]
    lines.extend(_fmt(r) for r in rows[:60])
    lines.append("")
    lines.append(f"best_accepted: {_fmt(accepted[0]) if accepted else 'none'}")
    lines.append(f"best_goal: {_fmt(goal[0]) if goal else 'none'}")
    lines.append(f"best_positive_score: {_fmt(positive[0]) if positive else 'none'}")
